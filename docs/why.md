...
`go-ruby-activestorage/activestorage` reimplements Ruby's `activestorage` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-activestorage/activestorage` directly, with no Ruby runtime;
...
Because `activestorage` is pure Go with cgo disabled, it also compiles to **WebAssembly**
